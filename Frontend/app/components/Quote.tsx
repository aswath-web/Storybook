...
function Quote({setFlagLike,flagLike}:CreateProps) {
  ...
  const createPost = async () => {
    try {
      const blobStory = new Blob([quote], { type: 'text/plain' });
      ...
      if (ethereum) {
        ...
        const res = await AccountContract.addPosts(hash, "quote",'', genre);
  ...
  return (
    <div>
      <div className='flex items-center justify-around'>
        <h1 className='mt-4 text-white font-serif font-bold ml-28'>QUOTES</h1>
